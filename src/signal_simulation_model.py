 ...
 * ==========================================================================
 * Dieser Code kombiniert die industrielle Logik mit der Testbench
 * für die Ausführung in OnlineGDB oder auf dem PC.
 */
...
typedef struct { ZCS_State state; PhaseMode actual_hardware_phases; PhaseMode pending_phases; uint16_t timer; bool hardware_fault; uint8_t retry_counter; uint8_t consecutive_hw_failures; } HardwareController;
...
uint8_t FIR_Process(FIR_FilterState* state, int16_t in_sample, const int16_t* coeffs, int16_t* out_sample) {
    SAFE_ASSERT(state != NULL); SAFE_ASSERT(coeffs != NULL);
    ...
    if (state->decimation_counter >= DECIMATION_M) {
        ...
        if (result > 32767) result = 32767; else if (result < -32768) result = -32768;
...
void Hysteresis_Init(PhaseHysteresis* hyst) {
    SAFE_ASSERT(hyst != NULL); hyst->current_state = PHASES_1; hyst->timer_up = 0; hyst->timer_down = 0;
}
...
PhaseMode Hysteresis_Process(PhaseHysteresis* hyst, int32_t pv_surplus_w) {
    ...
    if (hyst->current_state == PHASES_1) {
        if (pv_surplus_w >= THRESHOLD_1_TO_3_W) {
            hyst->timer_up++;
            if (hyst->timer_up >= DELAY_UP_SECONDS) { hyst->current_state = PHASES_3; hyst->timer_up = 0; }
        } else hyst->timer_up = 0;
    } else if (hyst->current_state == PHASES_3) {
        if (pv_surplus_w <= THRESHOLD_3_TO_1_W) {
            hyst->timer_down++;
            if (hyst->timer_down >= DELAY_DOWN_SECONDS) { hyst->current_state = PHASES_1; hyst->timer_down = 0; }
        } else hyst->timer_down = 0;
    }
...
ChargeCommand Calculate_Charge_Power(int32_t pv_surplus_w, PhaseMode active_phases) {
    ...
    if (active_phases != PHASES_1 && active_phases != PHASES_3) { cmd.active_phases = PHASES_OFF; return cmd; }
...
void HardwareController_Init(HardwareController* hw) {
    SAFE_ASSERT(hw != NULL); hw->state = ZCS_IDLE; hw->actual_hardware_phases = PHASES_1; hw->consecutive_hw_failures = 0;
...
void Execute_Hardware_Command(HardwareController* hw, ChargeCommand cmd) {
    ...
    switch (hw->state) {
        case ZCS_IDLE:
            if (cmd.active_phases != hw->actual_hardware_phases) {
                if (Set_PWM_Output_Amps(0)) { hw->state = ZCS_WAIT_CAR_STOP; hw->timer = 0; hw->pending_phases = cmd.active_phases; hw->consecutive_hw_failures = 0; }
                else { hw->hardware_fault = true; hw->consecutive_hw_failures++; }
            } else {
                if (Set_PWM_Output_Amps(cmd.target_current_da)) { hw->consecutive_hw_failures = 0; }
                else { hw->hardware_fault = true; hw->consecutive_hw_failures++; }
            }
            break;
        case ZCS_WAIT_CAR_STOP:
            ...
            if (hw->timer >= CAR_RAMPDOWN_TICKS) {
                if (Set_Relays(hw->pending_phases)) { hw->actual_hardware_phases = hw->pending_phases; hw->state = ZCS_WAIT_RELAY_SETTLE; hw->timer = 0; hw->consecutive_hw_failures = 0; }
                else { hw->consecutive_hw_failures++; }
            }
            break;
        case ZCS_WAIT_RELAY_SETTLE:
            hw->timer++;
            if (hw->timer >= RELAY_SETTLE_TICKS) { hw->state = ZCS_IDLE; }
            break;
    }
}

ISR(TIMER1_COMPA_vect) { if (timer_flag) timer_overrun = true; timer_flag = true; }

void* wallbox_thread(void* arg) {
    wdt_enable(WDTO_2S); FIR_Init(&pv_filter); Hysteresis_Init(&phase_hysteresis); HardwareController_Init(&hw_controller);
    uint8_t tick_counter_1sec = 0; uint8_t sensor_error_counter = 0; ChargeCommand current_cmd = {PHASES_1, 0};
    int16_t smooth_pv = 0; int16_t last_valid_pv = 0;
    sei();
    while(1) {
        if (timer_flag) {
            timer_flag = false; timer_overrun = false;
            int16_t raw_pv = Read_PV_Surplus_Watts();
            if (Validate_PV_Value(raw_pv, &sensor_error_counter)) last_valid_pv = raw_pv; else last_valid_pv = 0;
            if (sensor_error_counter >= PV_MAX_ERRORS || hw_controller.hardware_fault) { current_cmd.active_phases = PHASES_OFF; current_cmd.target_current_da = 0; }
            else {
                FIR_Process(&pv_filter, last_valid_pv, fir_coeffs, &smooth_pv);
                tick_counter_1sec++;
                if (tick_counter_1sec >= 10) { tick_counter_1sec = 0; PhaseMode target_phases = Hysteresis_Process(&phase_hysteresis, (int32_t)smooth_pv); current_cmd = Calculate_Charge_Power((int32_t)smooth_pv, target_phases); }
            }
            Execute_Hardware_Command(&hw_controller, current_cmd);
            if (hw_controller.consecutive_hw_failures < 10) wdt_reset();
        }
        usleep(100);
    }
    return NULL;
...
int16_t Read_PV_Surplus_Watts(void) {
    if (sim_tick < 500) return 1200;  // 1.2kW
    if (sim_tick < 1500) return 6000; // 6kW -> Upgrade
    return 1500;                     // Cloud -> Downgrade
}
bool Set_PWM_Output_Amps(int16_t da) { if (i2c_dead) return false; printf("[ACT] PWM: %d.%dA\n", da/10, da%10); return true; }
bool Set_Relays(PhaseMode p) { if (i2c_dead) return false; printf("[ACT] Relais: %d Phasen\n", p); return true; }

int main() {
    printf("--- Wallbox V2.6 Simulation startet ---\n");
    pthread_t thread;
    pthread_create(&thread, NULL, wallbox_thread, NULL);
    uint32_t last_wdt = 0;

    for (sim_tick = 0; sim_tick < 2500; sim_tick++) {
        if (sim_tick == 2200) { printf("\n[!] SIM: I2C BUS CRASH!\n"); i2c_dead = true; }
        TIMER1_COMPA_vect();
        usleep(5000); // 5ms Realzeit pro 100ms Sim-Tick
        if (sim_tick % 100 == 0) {
            printf("[T=%2ds] PV: %dW | Filter: %dW | WDT-Feeds: %d\n", sim_tick/10, Read_PV_Surplus_Watts(), 0, wdt_feed_count - last_wdt);
            if (wdt_feed_count == last_wdt && i2c_dead) { printf("\n[SUCCESS] Watchdog hat System-Reset ausgeloest!\n"); exit(0); }
